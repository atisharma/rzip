% a function which is a flag for a given machine. this is the local IC one, the EPFL one would be v different but is already in
% existence in Lausanne. 

function [R_AC,Z_AC,RES_AC,DR_AC,DZ_AC,LA_AC,NT_AC,R_PA,Z_PA,RES_PA,DR_PA,DZ_PA,LA_PA,R_BP,Z_BP,TH_BP,LA_BP,R_FL,Z_FL,LA_FL]=tcv();

% Set up the maximum number of eigenmodes used array in here
  global N_eigen_modes_array_max

  N_eigen_modes_array_max = [40];

% load ./tcv_magnetics;       %(original file sent for distribution)
% load ./Basic_data;  %(has no filaments or cross sections, this does).
  load tcv_structure;       %(original file sent for distribution)
...
% no DR and DZ for gcoils so bluff them
  DR_AC(g_coils)=2*sqrt(fil_cs(g_coils)/pi);
  DZ_AC(g_coils)=2*sqrt(fil_cs(g_coils)/pi);
