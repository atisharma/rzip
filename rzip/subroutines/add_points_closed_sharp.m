...
% where the first and last point are the same. This routine
% uses linear interpolation for vertical distances under
% -4m, so as to better follow the diivertor.

function [RR,ZZ,XX,YY,NN]=add_points_closed_sharp(fil_nam,NP);
...
% Throw in the quick check for less than 4 metres. Only z can be negative
  if(min([X_test Y_test])< -4);
   mlin=(Y_test(2)-Y_test(1))/(X_test(2)-X_test(1));cons=Y_test(2)-mlin*X_test(2);
   YIN=mlin*XIN+cons;
  end;
