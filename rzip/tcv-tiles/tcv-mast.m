% TCV description with conducting tiles and vacuum vessel moved outside the coils, 

function [R_AC,Z_AC,RES_AC,DR_AC,DZ_AC,LA_AC,NT_AC,R_PA,Z_PA,RES_PA,DR_PA,DZ_PA,LA_PA,R_BP,Z_BP,TH_BP,LA_BP,R_FL,Z_FL,LA_FL]=tcv_mast();
...
% PASSIVE STRUCTURE
 ...
 R_LS=find(R_PA<.8);
 R_GE=find(R_PA>=.8);
 ORNTZ=find(DZ_PA>DR_PA);
 ORNTR=find(DZ_PA<=DR_PA);

 R_PA(R_LS)=R_PA(R_LS)./(1+2.0*r_scale*(0.8-R_PA(R_LS))./R_PA(R_LS));
 R_PA(R_GE)=R_PA(R_GE).*(1+r_scale*(R_PA(R_GE)-0.8)./R_PA(R_GE));
 DZ_PA(ORNTZ)=DZ_PA(ORNTZ)*z_scale;
 DR_PA(ORNTR)=DR_PA(ORNTR)*z_scale;
 Z_PA=Z_PA*z_scale;
...
%   tile_range=[];
  ...
  RES_PA=[RES_PA;RES_TL(tile_range)'/5];
