...
% Set up the maximum number of eigenmodes used array in here
  ...
  N_eigen_modes_array_max = [40 40 40];
...
% add an external vacuum vessel
 ...
 R_V2(R_LS)=R_PA(R_LS)./(1+2.0*r_scale*(0.8-R_PA(R_LS))./R_PA(R_LS));
 R_V2(R_GE)=R_PA(R_GE).*(1+r_scale*(R_PA(R_GE)-0.8)./R_PA(R_GE));
 DZ_V2(ORNTZ)=DZ_PA(ORNTZ)*z_scale;
 DR_V2(ORNTR)=DR_PA(ORNTR)*z_scale;
 Z_V2=Z_PA*z_scale;
  ...
  DR_PA =[DR_PA;DR_V2];
  DZ_PA =[DZ_PA;DZ_V2'];
  RES_PA=[RES_PA;RES_PA];

 ff='00';
 for k=1:length(R_V2);
  if(k>=10&k<99);ff='0';end;  if(k>=100);ff='';end;
  eval(['LA_PA=[LA_PA;''V2',ff,num2str(k,3),'F001''];']);
 end;
