...
% where the first and last point are not the same.

function [RR,ZZ,XX,YY,NN]=add_points_closed(RC,ZC,NP);
...
% determine the local gradient. If too steep need to consider Z as variable
  if((RCT(k1)-RCT(k0)) & (RCT(k2)-RCT(k1))); 
   grad_line=(ZCT(k1)-ZCT(k0))/(RCT(k1)-RCT(k0));
   grad_line2=(ZCT(k2)-ZCT(k1))/(RCT(k2)-RCT(k1));
  else;
   grad_line=1000;
   grad_line2=1000;
  end;
  if(abs(grad_line)<1 | ~grad_line2)  
