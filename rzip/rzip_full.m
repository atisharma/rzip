...
% Based on the RZIP concept and methods as outlined in Nuclear Fusion 39(5) 663
%
% [A, B, C, D, curlyM, curlyR, plasma_outputs, IFLAG] = 
%           rzip(shot, time, Used_coils, N_eigen_modes, plasma_inputs, Machine_name, force_make, extra_routine)
%
% Inputs
% ======
...
% N_eigen_modes,     How many eigenvalues to use.
...
% global variables associated with workspace 
  global Z_plasma_old R_plasma_old Machine_name_old N_eigen_modes_old Used_coils_old
...
% always assume a local file.
  Machine_name_path=pwd;
   
% define a path separation variable (main exception being VMS)
  path_fill = filesep; if(path_fill=='.'); path_fill=''; end;   

% Create two file name variables to the output files.
  plasma_less_file  = [Machine_name_path,path_fill,Machine_name,'_plasma_less_v2.mat'];
  plasma_full_file  = [Machine_name_path,path_fill,Machine_name,'_plasma_full_v2.mat'];
 
% default the initial 'old' variables
  if(~length(Machine_name_old));
   Machine_name_old = '   ';  N_eigen_modes_old = 0;  Used_coils_old = [];
  end
...
% will not reload unless i) N_eigen_modes has changed, ii) Used_coils are different     
%======================================================================================================================== 
...
%------------------------------------------------------------- 
  ...
  if(fid == -1); make_plasma_less = 1; end; 
  fclose(fid);
...
% however all above is for naught if force_make is defined
  if(force_make);   
   if(force_make~=99); make_plasma_less = 1; do_not_load_less = 0; end;
   if(~plasma_less);   make_plasma_full = 1; do_not_load_full = 0; end; 
...
%-------------------------------- 
  if(screen_is_on); disp('Found no saved plasmaless data; creating from scratch'); end;
...
% seek out the different structures by name
 ...
 if(screen_is_on);
  ...
  disp(['will use ',fill,' Active coils and ',num2str(N_eigen_modes),' eigenmodes']);
...
% define eigenfunctions (vectors Ee and values Ve)
 inv_R   = inv(diag(RES_PS));
 inv_R_L = inv_R*L_pf_pf;
 [Ee,Ve] = eig(inv_R_L);
...
% set maximum number of emodes
  if(N_eigen_modes_max > N_passive_fils)
   N_eigen_modes_max = N_passive_fils; 
   if(screen_is_on); disp('More eigenmodes than points, have reset N_eigen_modes_max'); end;
  end;  

% define the reduced set of eigenfunctions (vectors Eigen_vectors and values Eigen_values)
  Eigen_vectors = Es(:,1:N_eigen_modes_max); 
  Eigen_values  = Vs(1:N_eigen_modes_max,1:N_eigen_modes_max); 
...
%--------------------------------------
  ...
  Res_struct = [          Res_ac_ac                          zeros(N_active_coils,N_eigen_modes_max) ;
                 zeros(N_eigen_modes_max,N_active_coils)         norm_res*eye(size(Eigen_values))   ]; 
...
% Save the necessary outputs for later
  ...
  feval('save',plasma_less_file, ...
               'N_eigen_modes_max','N_passive_fils','N_active_coils','N_active_fils', ...  
...
% check the current number of eigenmodes is less than the max
  if( N_eigen_modes > N_eigen_modes_max)
   N_eigen_modes = N_eigen_modes_max;
   if(screen_is_on); disp(['N_eigen_modes larger than this saved data allows, reset to ',num2str(N_eigen_modes_max)]); end;
   end;
  end;
...
% --------------------------------------------------------------------------------------
 if(do_not_load_less)
                ...
                Used_circuits = [1:(N_Used_coils+N_eigen_modes)];
 else
                Used_circuits = [Used_coils,N_active_coils+1:N_active_coils+N_eigen_modes];
                 N_Used_coils = length(Used_coils);
...
% tell the user what's happening
%-------------------------------- 
 if(screen_is_on);    disp('Found no saved plasmafull data; creating from scratch');     end;
...
% set up the 'old' variables for use in comparing present and previous execution
  Machine_name_old = Machine_name;  N_eigen_modes_old = N_eigen_modes;   Used_coils_old =  Used_coils;
...
% Useful equilibrium terms
 ...
 index_ps   = N_Used_coils+1:N_Used_coils+N_eigen_modes;
 Fs         = dL_plas_struct_dz(index_ps)'*(dL_plas_struct_dz(index_ps)./diag(L_struct(index_ps,index_ps)))*IP*IP; 
...
% If we have disturbances then we need to rearrange things
 if(make_disturb)
  e_dist = -mu0*IP*IP/2;
  E      = minv*[zeros(N_Used_coils+N_eigen_modes,2); e_dist e_dist/2; 0 0];
