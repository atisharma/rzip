...
% the bgbm actually requires Lp etc so we must save local data to speed up the process.
% Equilibrium data is save in a shot file with time arrays.

% Potential Lp plasma equilibrium file is 
  Lp_file  = [Machine_name,'_Lp_',num2str(shot),'_v3.mat'];
 
% if not a local file check if is in the path structure somewhere.
  remote_Lp_file=which(Lp_file);
  if(length(remote_Lp_file)); Lp_file = remote_Lp_file; end;
      
% if file exists check how many entries   
  Lp_fid = fopen(Lp_file,'r');   
  if(Lp_fid >= 0);
   fclose(Lp_fid);
   feval('load',Lp_file);
   N_in_file = find(Lp_times==time);if(~length(N_in_file)); N_in_file = 0 ; end;
  else;
   N_in_file = 0; Lp_times =[]; Lps = []; dLps = []; d2Lps = []; Bds = [];
  end;
...
% work out plasma inductance terms 'Exactly'
  ...
  if(screen_is_on);
   disp(['Evaluating plasma inductance terms took ',num2str(find_time-ind_time),' seconds']);
   if(Lp_fid>=0); 
    disp(['Appending equilibrium file ',Lp_file]); 
    feval('load',Lp_file);
   else ;
    disp(['Creating equilibrium file ',Lp_file]); 
   end;
  end;
   
  Lp_times = [Lp_times;time];
  Lps      = [Lps;Lp0];
  dLps     = [dLps;dLp_dR0];
  d2Lps    = [d2Lps;d2Lp_dR02];
  Bds      = [Bds;Bdc];
   
  feval('save',Lp_file,'Lp_times','Lps','dLps','d2Lps','Bds');
   
 else % if equilibrium data file exists then reload it  (bgbm loop)
    
  feval('load',Lp_file);
  Lp0       = Lps(N_in_file);
  dLp_dR0   = dLps(N_in_file);
  d2Lp_dR02 = d2Lps(N_in_file);
  Bdc       = Bds(N_in_file);
...
if(make_disturb)
   ...
   rad_dist = -IP/2*mu0;
   IP_dist = -mu0*R0*IP;
   E      = minv*[zeros(N_Used_coils+N_passive_fils+1,2); 0 rad_dist; 0 IP_dist];
